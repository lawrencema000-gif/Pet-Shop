    ...
    <>
      ...
      <div className="border-b border-border bg-surface-light">
        <nav className="container-main flex items-center gap-2 py-3 text-sm text-muted">
          ...
          <span className="text-foreground">{t('sale.breadcrumb')}</span>
        </nav>
      </div>
      ...
      <section className="border-b border-border bg-surface-light">
        <div className="container-main grid grid-cols-2 gap-4 py-4 md:grid-cols-4">
          {[
            { icon: "\uD83D\uDE9A", textKey: "sale.trustFreeShipping" },
            { icon: "\uD83D\uDD04", textKey: "sale.trustReturns" },
            { icon: "\uD83D\uDEE1\uFE0F", textKey: "sale.trustWarranty" },
            { icon: "\uD83D\uDCB3", textKey: "sale.trustCheckout" },
          ].map((badge) => (
            <div
              key={badge.textKey}
        ...
        <h2 className="mb-8 text-2xl font-bold text-foreground">
          ...
          {saleProducts.length > 0 && (
            <span className="ml-2 text-base font-normal text-muted">
              ({t('sale.itemsCount', { count: saleProducts.length })})
            </span>
          )}
        </h2>
