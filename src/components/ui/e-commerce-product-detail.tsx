        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8">
          ...
          <div className="relative">
            <div className="relative aspect-square overflow-hidden rounded-xl bg-surface mb-4">
              {currentImage?.url ? (
                ...
                <div className="flex items-center justify-center h-full text-muted">No image available</div>
              )}
              {images.length > 1 && (
                <>
                  <button onClick={prevImage} className="absolute top-1/2 left-4 -translate-y-1/2 bg-white/90 rounded-full p-2 shadow-elevated hover:bg-white transition-colors focus:outline-none focus:ring-2 focus:ring-accent" aria-label="Previous image"><ArrowLeft size={20} className="text-foreground" /></button>
                  <button onClick={nextImage} className="absolute top-1/2 right-4 -translate-y-1/2 bg-white/90 rounded-full p-2 shadow-elevated hover:bg-white transition-colors focus:outline-none focus:ring-2 focus:ring-accent" aria-label="Next image"><ArrowRight size={20} className="text-foreground" /></button>
                </>
              )}
            </div>
            {images.length > 1 && (
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => setIsWishlisted(!isWishlisted)} className={cn("p-2 rounded-full transition-colors", isWishlisted ? "text-sale bg-sale/10" : "text-muted hover:text-sale")} aria-label={isWishlisted ? "Remove from wishlist" : "Add to wishlist"}>
              ...
              <div className="flex items-center gap-2">
                {product.rating_count > 0 && (
                  <>
                    ...
                    <a href="#reviews" className="text-sm text-muted hover:text-foreground transition-colors">({product.rating_count} reviews)</a>
                  </>
                )}
              </div>
              <button onClick={handleShare} className="flex items-center gap-1.5 text-sm text-muted hover:text-foreground transition-colors"><Share2 size={16} /> Share</button>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={cn("text-sm font-medium", stockStatus === "in" ? "text-success" : stockStatus === "low" ? "text-orange-500" : "text-red-500")}>
                {stockStatus === "in" ? "In Stock" : stockStatus === "low" ? `Low Stock — Only ${stockQuantity} left!` : "Out of Stock"}
              </span>
            </div>
            ...
            <div className="min-h-[120px]">
              ...
              {activeTab === "details" && <div className="text-sm text-muted">{specsText ? <div className="whitespace-pre-wrap">{specsText}</div> : <p>No additional details available.</p>}</div>}
              {activeTab === "shipping" && (
                <div className="space-y-4">
                  <div className="flex items-center"><Truck size={20} className="text-accent mr-3" /><div><h3 className="font-medium text-foreground">Free shipping</h3><p className="text-sm text-muted">Est. {fmtDate(deliveryStart)} – {fmtDate(deliveryEnd)}</p></div></div>
                  <div className="flex items-center"><ShieldCheck size={20} className="text-accent mr-3" /><div><h3 className="font-medium text-foreground">30-day returns</h3><p className="text-sm text-muted">Hassle-free returns</p></div></div>
                </div>
              )}
            </div>
            ...
            <div className="pt-2">
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-border rounded-md">
                  <button onClick={() => quantity > 1 && setQuantity(quantity - 1)} className="p-2.5 text-muted hover:text-foreground transition-colors disabled:opacity-40" disabled={quantity <= 1} aria-label="Decrease quantity"><Minus size={18} /></button>
                  ...
                  <button onClick={() => setQuantity(Math.min(quantity + 1, stockQuantity))} className="p-2.5 text-muted hover:text-foreground transition-colors disabled:opacity-40" disabled={stockStatus === "out"} aria-label="Increase quantity"><Plus size={18} /></button>
                </div>
                <button type="button" onClick={handleAddToCart} disabled={stockStatus === "out"} className={cn("flex-1 py-3 px-6 rounded-md flex items-center justify-center gap-2 font-medium transition-all", stockStatus === "out" ? "bg-surface text-muted cursor-not-allowed" : addedToCart ? "bg-success text-white" : "bg-accent text-white hover:bg-accent-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent")}>
                  {addedToCart ? (<><Check size={20} /> Added!</>) : stockStatus === "out" ? "Out of Stock" : (<><ShoppingBag size={20} /> Add to cart</>)}
                </button>
              </div>
            ...
            <div className="flex items-center gap-2 text-sm text-muted"><Truck size={16} className="text-accent" /><span>Estimated delivery: <span className="font-medium text-foreground">{fmtDate(deliveryStart)} – {fmtDate(deliveryEnd)}</span></span></div>

            {/* Trust Badges */}
            <div className="grid grid-cols-3 gap-3 pt-4 border-t border-border">
              {[{ icon: Truck, label: "Free Shipping", sub: "Orders over $75" }, { icon: RotateCcw, label: "30-Day Returns", sub: "Hassle-free" }, { icon: ShieldCheck, label: "1-Year Warranty", sub: "Full coverage" }].map(({ icon: Icon, label, sub }) => (
